...
{
    /** @var callable|null Injected command runner — used in tests to avoid real exec() calls */
    protected $commandRunner = null;

    private bool   $coverageAutoGenerated    = false;
    private ?string $coverageGenerationWarning = null;
    ...
    public function analyze(string $projectPath): array
    {
        ...
        $this->coverageAutoGenerated     = false;
        $this->coverageGenerationWarning = null;
        ...
        // 4. Check for coverage XML report (auto-generates if missing)
        $coverageData = $this->parseCoverageReport($projectPath);
        $metrics['coverage_xml_found']          = $coverageData !== null;
        $metrics['coverage_auto_generated']     = $this->coverageAutoGenerated;
        $metrics['coverage_generation_warning'] = $this->coverageGenerationWarning;
        $metrics['line_coverage'] = $coverageData['line_coverage'] ?? 'Not available';
    ...
    private function parseCoverageReport(string $projectPath): ?array
    {
        ...
        return $this->attemptCoverageGeneration($projectPath);
    }

    private function attemptCoverageGeneration(string $projectPath): ?array
    {
        $phpunitBin = $projectPath . '/vendor/bin/phpunit';

        if (!file_exists($phpunitBin)) {
            return null;
        }

        $cloverOutput = $projectPath . '/build/coverage/clover.xml';
        $outputDir    = dirname($cloverOutput);

        if (!is_dir($outputDir)) {
            mkdir($outputDir, 0777, true);
        }

        $result = $this->runCoverageCommand($projectPath, $cloverOutput);

        if ($result['exit_code'] !== 0) {
            $this->coverageGenerationWarning =
                'Coverage generation ran but tests failed — fix failing tests to get coverage data. ' .
                trim(substr($result['output'], -300));
            return null;
        }

        if (!file_exists($cloverOutput)) {
            return null;
        }

        $this->coverageAutoGenerated = true;

        return $this->parseCloverXml($cloverOutput);
    }

    protected function runCoverageCommand(string $projectPath, string $cloverPath): array
    {
        $command = sprintf(
            'cd %s && php vendor/bin/phpunit --coverage-clover=%s --no-interaction 2>&1',
    ...
    private function generateRecommendations(array $metrics): void
    {
        ...
        if (!$metrics['coverage_xml_found']) {
            if ($metrics['coverage_generation_warning'] !== null) {
                $this->addRecommendation("Coverage auto-generation failed because tests are not passing. Fix failing tests first, then re-run the analyzer.");
            } else {
                $this->addRecommendation("Install Xdebug or PCOV to enable coverage: 'php artisan test --coverage-clover=coverage.xml'. Aim for at least 70%.");
            }
        } elseif ($metrics['coverage_auto_generated']) {
            $this->addRecommendation("Coverage was auto-generated during analysis. Consider committing the clover.xml to your CI artifacts for faster runs.");
        }
